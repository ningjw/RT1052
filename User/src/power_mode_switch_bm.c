 ...
 * Variables
 ******************************************************************************/
uint8_t s_wakeupTimeout;            /* Wakeup timeout. (Unit: Second) */
app_wakeup_source_t s_wakeupSource; /* Wakeup source.                 */
...
void APP_PowerPreSwitchHook(lpm_power_mode_t targetMode)
{
    ...
    if (targetMode > LPM_PowerModeRunEnd)
    {
        ...
        while (!(kLPUART_TransmissionCompleteFlag & LPUART_GetStatusFlags((LPUART_Type *)LPUART2)))
        {
        }
        DbgConsole_Deinit();
