                                              ...
                                              usb_host_transfer_t *transfer)
{
    ...
    {
        ehciInstance->ehciQtdNumber -= qtdNumber;
        BaseQtdPointer = ehciInstance->ehciQtdHead;//BaseQtdPointer 指向链表头
        qtdPointer = NULL;
        do//将 ehciInstance->ehciQtdHead 指向链表尾
        {
            if (qtdPointer != NULL)
            {
                qtdPointer->nextQtdPointer = (uint32_t)ehciInstance->ehciQtdHead;
            }
            qtdPointer = ehciInstance->ehciQtdHead;//将qtd的头地址 保存在 qtdPointer 指针中
            ehciInstance->ehciQtdHead = (usb_host_ehci_qtd_t *)qtdPointer->nextQtdPointer;//将头指向 链表中下一个
            qtdPointer->nextQtdPointer = 0; //头指针的下一个清零
        } while (--qtdNumber);
    ...
    /* int qTD list */
    if (ehciPipePointer->pipeCommon.pipeType == USB_ENDPOINT_CONTROL)//控制传输
    {
        /* setup transaction qtd 建立过程qtd初始化*/
    ...
    /* save qtd to transfer */
    transfer->union1.unitHead = (uint32_t)BaseQtdPointer;//qtd头
    transfer->union2.unitTail = (uint32_t)qtdPointer;
    /* link transfer to qh */
    transfer->next = NULL;
    if (vltQhPointer->ehciTransferHead == NULL)
        ...
        kStatus_USB_OSA_Success) /* wait all event */
    {
        ...
        else if (ehciInstance->deviceAttached != kEHCIDeviceAttached)
        {
            if (bitSet & EHCI_TASK_EVENT_DEVICE_ATTACH) /* device is attached */
            {
                ...
                USB_HostEhciStartPeriodic(ehciInstance);
                //进行一系列的初始化
                if (USB_HostAttachDevice(ehciInstance->hostHandle, ehciInstance->firstDeviceSpeed, 0, portNum++, 1,
